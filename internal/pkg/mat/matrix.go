...
import (
	...
	"github.com/brentshierk/raytracer/internal/pkg/identity"
)
...
func MultiplyByTuple(m1 Mat4x4, t Set) Set {
	t1 := NewSet()
	for row := 0; row < 4; row++ {
		a := m1[(row*4)+0] * t[0]
		b := m1[(row*4)+1] * t[1]
...
func MultiplyByTupleUsingValues(m1 Mat4x4, t Set, out *Set) {
	for row := 0; row < 4; row++ {
		a := m1[(row*4)+0] * t[0]
		b := m1[(row*4)+1] * t[1]
		c := m1[(row*4)+2] * t[2]
...
func MultiplyByTupleUsingPointers(m1 *Mat4x4, t, out *Set) {
	for row := 0; row < 4; row++ {
		a := m1[(row*4)+0] * t[0]
		b := m1[(row*4)+1] * t[1]
		c := m1[(row*4)+2] * t[2]
